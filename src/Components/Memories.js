...
const Memories = () => {
    ...
    return (
        <div className="min-h-screen flex flex-col overflow-x-hidden overflow-y-auto animate-fadeIn">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
                ...
                <div className="flex flex-wrap justify-between">
                    {images.map((image, index) => (
                        <figure
                            key={index}
                            className="w-full md:w-1/2 lg:w-1/3 card mb-8 md:mb-0 transform transition-transform hover:scale-103 hover:shadow-md px-4 animate-fadeIn"
                            onClick={() => openModal(index)}
                        >
                            <img src={image.src} alt="memories" className="w-full h-full object-cover filter drop-shadow-lg py-3 transition-opacity duration-300" />
                        </figure>
                    ))}
                </div>
            </div>
            {modalOpen && (
                <div className="fixed top-0 left-0 w-full h-full bg-black bg-opacity-75 flex justify-center items-center z-50 animate-fadeIn">
                    <div className="max-w-xl w-full relative">
                        ...
                        <img src={images[selectedImageIndex].src} alt="memories" className="w-full h-auto animate-fadeIn" />
                    </div>
                </div>
            )}
        </div>
