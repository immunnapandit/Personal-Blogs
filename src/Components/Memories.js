            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
                ...
                <div className="flex flex-wrap justify-between">
                    {images.map((image,index) =>(
                        <figure key={index} className="w-full md:w-1/2 lg:w-1/3 card mb-8 md:mb-0 transform transition-transform hover:scale-103 hover:shadow-md px-4"
                        onClick={()=> openModal(index)}>
                            <img src={image} alt="memories" className="w-full h-full object-cover filter drop-shadow-lg py-3" />
                        </figure>
                    ))}
                </div>
            </div>
            {modalOpen &&(
                 <div className="fixed top-0 left-0 w-full h-full bg-black bg-opacity-75 flex justify-center items-center z-50">
                    <div className="max-w-xl w-full relative">
                        ...
                        <button onClick={prevImages} className="absolute left-0 top-1/2 transform -translate-y-1/2 text-white text-2xl font-bold" style={{ backgroundColor: 'rgba(0,0,0,0.5)', padding: '8px 16px', borderRadius: '4px' }}>&#8249;</button>
                        ...
                        <img src={images[selectedImageIndex]} alt="memories" className="w-full h-auto" />
                    </div>
                </div>
            )}
        </div>
